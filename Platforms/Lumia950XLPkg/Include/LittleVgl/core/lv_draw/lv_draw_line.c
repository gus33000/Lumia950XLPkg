 ...
 *  STATIC PROTOTYPES
 **********************/
static void line_draw_hor(
    line_draw_t *main_line, const lv_area_t *mask, const lv_style_t *style,
    lv_opa_t opa_scale);
static void line_draw_ver(
    line_draw_t *main_line, const lv_area_t *mask, const lv_style_t *style,
    lv_opa_t opa_scale);
static void line_draw_skew(
    line_draw_t *main_line, const lv_area_t *mask, const lv_style_t *style,
    lv_opa_t opa_scale);
static void
            line_init(line_draw_t *line, const lv_point_t *p1, const lv_point_t *p2);
 ...
 */
void lv_draw_line(
    const lv_point_t *point1, const lv_point_t *point2, const lv_area_t *mask,
...
static void line_draw_hor(
    line_draw_t *line, const lv_area_t *mask, const lv_style_t *style,
    lv_opa_t opa_scale)
{
    ...
    lv_opa_t opa_scale)
{
  ...
  /* In case of a short but tick line the perpendicular ending is longer then
   * the real line. it would break the calculations so make the normal vector
   * larger*/
  ...
  /* The pattern stores the points of the line ending. It has the good direction
   * and length. The worth case is the 45° line where pattern can have 1.41 x
   * `width` points*/
  ...
  if (width != 0) {
    ...
    /* Run for a lot of times. Meanwhile the real width will be determined as
     * well */
    for (i = 0; i < (lv_coord_t)sizeof(pattern); i++) {
      pattern[i].x = pattern_line.p_act.x;
      pattern[i].y = pattern_line.p_act.y;

      /*Finish the pattern line if it's length equal to the desired width (Use
       * Pythagoras theorem)*/
    ...
    if (i != 0) {
      if (main_line->hor) {
        if (pattern[i - 1].x != pattern[i].x) {
          ...
          if (main_line->sy < 0) {
            lv_draw_aa_ver_seg(
                main_line->p1.x + pattern[aa_last_corner].x - 1,
                main_line->p1.y + pattern[aa_last_corner].y + seg_w + 1, seg_w,
                mask, style->line.color, opa);

            lv_draw_aa_ver_seg(
                main_line->p2.x + pattern[aa_last_corner].x + 1,
                main_line->p2.y + pattern[aa_last_corner].y + seg_w + 1, -seg_w,
                mask, style->line.color, opa);
          }
          else {
            lv_draw_aa_ver_seg(
        ...
        if (pattern[i - 1].y != pattern[i].y) {
          ...
          if (main_line->sx < 0) {
            lv_draw_aa_hor_seg(
                main_line->p1.x + pattern[aa_last_corner].x + seg_w + 1,
                main_line->p1.y + pattern[aa_last_corner].y - 1, seg_w, mask,
                style->line.color, opa);

            lv_draw_aa_hor_seg(
                main_line->p2.x + pattern[aa_last_corner].x + seg_w + 1,
                main_line->p2.y + pattern[aa_last_corner].y + 1, -seg_w, mask,
                style->line.color, opa);
          }
          else {
            lv_draw_aa_hor_seg(
                main_line->p1.x + pattern[aa_last_corner].x,
                main_line->p1.y + pattern[aa_last_corner].y - 1, seg_w, mask,
                style->line.color, opa);

            lv_draw_aa_hor_seg(
                main_line->p2.x + pattern[aa_last_corner].x,
                main_line->p2.y + pattern[aa_last_corner].y + 1, -seg_w, mask,
                style->line.color, opa);
          }
          aa_last_corner = i;
  ...
  if (main_line->hor) {
    ...
    if (main_line->sy < 0) {
      ...
      lv_draw_aa_ver_seg(
          main_line->p1.x + pattern[aa_last_corner].x - 1,
          main_line->p1.y + pattern[aa_last_corner].y, seg_w + main_line->sy,
          mask, style->line.color, opa);

      lv_draw_aa_ver_seg(
          main_line->p2.x + pattern[aa_last_corner].x + 1,
          main_line->p2.y + pattern[aa_last_corner].y, -(seg_w + main_line->sy),
          mask, style->line.color, opa);
    }
  }
  else {
    ...
    if (main_line->sx < 0) {
      lv_draw_aa_hor_seg(
          main_line->p1.x + pattern[aa_last_corner].x + seg_w,
          main_line->p1.y + pattern[aa_last_corner].y - 1,
          seg_w + main_line->sx, mask, style->line.color, opa);

      lv_draw_aa_hor_seg(
          main_line->p2.x + pattern[aa_last_corner].x + seg_w,
  ...
  /*Shift the anti aliasing on the edges (-1, 1 or 0 (zero only in case width ==
   * 0))*/
  ...
  if (main_line->hor) {
    while (line_next_y(main_line)) {
      for (i = 0; i < width; i++) {
        ...
        /* Fill the gaps
         * When stepping in y one pixel remains empty on every corner (don't do
         * this on the first segment ) */
        if (i != 0 && pattern[i].x != pattern[i - 1].x && !first_run) {
          px_fp(
              draw_area.x1, draw_area.y1 - main_line->sy, mask,
              style->line.color, opa);
        }
      }
...
#if LV_ANTIALIAS
      lv_draw_aa_hor_seg(
          prev_p.x + pattern[0].x, prev_p.y + pattern[0].y - aa_shift1,
          -(main_line->p_act.x - prev_p.x), mask, style->line.color, opa);
      lv_draw_aa_hor_seg(
          prev_p.x + pattern[width_safe - 1].x,
    ...
    for (i = 0; i < width; i++) {
      ...
      if (i != 0 && pattern[i].x != pattern[i - 1].x && !first_run) {
        px_fp(
            draw_area.x1, draw_area.y1 - main_line->sy, mask, style->line.color,
            opa);
      }
    }
  ...
  /*Rather a vertical line*/
    ...
    while (line_next_x(main_line)) {
      for (i = 0; i < width; i++) {
        ...
        /* Fill the gaps
         * When stepping in x one pixel remains empty on every corner (don't do
         * this on the first segment ) */
        if (i != 0 && pattern[i].y != pattern[i - 1].y && !first_run) {
          px_fp(
              draw_area.x1 - main_line->sx, draw_area.y1, mask,
              style->line.color, opa);
        }
      }
...
#if LV_ANTIALIAS
      lv_draw_aa_ver_seg(
          prev_p.x + pattern[0].x - aa_shift1, prev_p.y + pattern[0].y,
          -(main_line->p_act.y - prev_p.y), mask, style->line.color, opa);
      lv_draw_aa_ver_seg(
          prev_p.x + pattern[width_safe - 1].x + aa_shift2,
          prev_p.y + pattern[width_safe - 1].y, main_line->p_act.y - prev_p.y,
          mask, style->line.color, opa);
#endif
    ...
    for (i = 0; i < width; i++) {
      ...
      if (i != 0 && pattern[i].y != pattern[i - 1].y && !first_run) {
        px_fp(
            draw_area.x1 - main_line->sx, draw_area.y1, mask, style->line.color,
            opa);
      }
    }
    ...
    lv_draw_aa_ver_seg(
        prev_p.x + pattern[width_safe - 1].x + aa_shift2,
        prev_p.y + pattern[width_safe - 1].y, main_line->p_act.y - prev_p.y + 1,
        mask, style->line.color, opa);
#endif
  }
}
...
line_init(line_draw_t *line, const lv_point_t *p1, const lv_point_t *p2)
{
  ...
  line->e2  = 0;
  line->hor =
      line->dx > line->dy ? true : false; /*Rather horizontal or vertical*/
...
static bool line_next(line_draw_t *line)
{
  if (line->p_act.x == line->p2.x && line->p_act.y == line->p2.y)
    return false;
  line->e2 = line->err;
  if (line->e2 > -line->dx) {
    line->err -= line->dy;
