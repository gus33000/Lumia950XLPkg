...
#if LV_VDB_SIZE != 0
void (*const px_fp)(
    lv_coord_t x, lv_coord_t y, const lv_area_t *mask, lv_color_t color,
    lv_opa_t opa) = lv_vpx;
void (*const fill_fp)(
    const lv_area_t *coords, const lv_area_t *mask, lv_color_t color,
    lv_opa_t opa) = lv_vfill;
void (*const letter_fp)(
    const lv_point_t *pos_p, const lv_area_t *mask, const lv_font_t *font_p,
    uint32_t letter, lv_color_t color, lv_opa_t opa) = lv_vletter;
void (*const map_fp)(
...
void (*const fill_fp)(
    const lv_area_t *coords, const lv_area_t *mask, lv_color_t color,
    lv_opa_t opa) = lv_rfill;
void (*const letter_fp)(
    const lv_point_t *pos_p, const lv_area_t *mask, const lv_font_t *font_p,
    uint32_t letter, lv_color_t color, lv_opa_t opa) = lv_rletter;
void (*const map_fp)(
    const lv_area_t *cords_p, const lv_area_t *mask_p, const uint8_t *map_p,
    lv_opa_t opa, bool chroma_key, bool alpha_byte, lv_color_t recolor,
    lv_opa_t recolor_opa) = lv_rmap;
#endif
...
lv_opa_t lv_draw_aa_get_opa(lv_coord_t seg, lv_coord_t px_id, lv_opa_t base_opa)
{
   ...
   * anti-aliasing? For example we have a line like this (y = -0.5 * x):
   *
   *  | _ _
   *    * * |
   *
   * Anti-aliased pixels come to the '*' characters
   * Calculate what percentage of the pixels should be covered if real line (not
   * rasterized) would be drawn:
   * 1. A real line should start on (0;0) and end on (2;1)
   * 2. So the line intersection coordinates on the first pixel: (0;0) (1;0.5)
   * -> 25% covered pixel in average
   ...
   * anti-aliased (was 2 in the example) px_id: pixel ID from 0 to
   * (segment_width - 1) result: [0..1] coverage of the pixel
   */
 ...
 */
void lv_draw_aa_ver_seg(
    lv_coord_t x, lv_coord_t y, lv_coord_t length, const lv_area_t *mask,
