...
#define VFILL_HW_ACC_SIZE_LIMIT                                                \
  50 /*Always fill < 50 px with 'sw_color_fill' because of the hw. init        \
        overhead*/
 ...
 *  STATIC PROTOTYPES
 **********************/
static void sw_mem_blend(
    lv_color_t *dest, const lv_color_t *src, uint32_t length, lv_opa_t opa);
static void sw_color_fill(
    lv_area_t *mem_area, lv_color_t *mem, const lv_area_t *fill_area,
    lv_color_t color, lv_opa_t opa);

#if LV_COLOR_SCREEN_TRANSP
static inline lv_color_t color_mix_2_alpha(
    lv_color_t bg_color, lv_opa_t bg_opa, lv_color_t fg_color, lv_opa_t fg_opa);
#endif
 ...
 */
void lv_vpx(
    lv_coord_t x, lv_coord_t y, const lv_area_t *mask_p, lv_color_t color,
    lv_opa_t opa)
{
  if (opa < LV_OPA_MIN)
    return;
 ...
 */
void lv_vfill(
    const lv_area_t *cords_p, const lv_area_t *mask_p, lv_color_t color,
    lv_opa_t opa)
{
  if (opa < LV_OPA_MIN)
    return;
  ...
  /*Not opaque fill*/
    ...
    /*Use hw blend if present and the area is not too small*/
        ...
        lv_area_get_height(&vdb_rel_a) > VFILL_HW_ACC_SIZE_LIMIT &&
 ...
 */
void lv_vletter(
    const lv_point_t *pos_p, const lv_area_t *mask_p, const lv_font_t *font_p,
    uint32_t letter, lv_color_t color, lv_opa_t opa)
{
  ...
  const uint8_t bpp2_opa_table[4]  = {0, 85, 170,
                                     255}; /*Opacity mapping with bpp = 2*/
  const uint8_t bpp4_opa_table[16] = {
  ...
  if (pos_x + letter_w < mask_p->x1 || pos_x > mask_p->x2 ||
      pos_y + letter_h < mask_p->y1 || pos_y > mask_p->y2)
    return;
  ...
  uint8_t     width_byte_scr =
      letter_w >> 3; /*Width in bytes (on the screen finally) (e.g. w = 11 -> 2
                        bytes wide)*/
  if (letter_w & 0x7)
    width_byte_scr++;
  uint8_t width_byte_bpp =
      (letter_w * bpp) >> 3; /*Letter width in byte. Real width in the font*/
  if ((letter_w * bpp) & 0x7)
    width_byte_bpp++;
  ...
  lv_coord_t col_end =
      pos_x + letter_w <= mask_p->x2 ? letter_w : mask_p->x2 - pos_x + 1;
  lv_coord_t row_start = pos_y >= mask_p->y1 ? 0 : mask_p->y1 - pos_y;
  lv_coord_t row_end =
      pos_y + letter_h <= mask_p->y2 ? letter_h : mask_p->y2 - pos_y + 1;

  /*Set a pointer on VDB to the first pixel of the letter*/
  vdb_buf_tmp +=
      ((pos_y - vdb_p->area.y1) * vdb_width) + pos_x - vdb_p->area.x1;
  ...
  for (row = row_start; row < row_end; row++) {
    ...
    for (col = col_start; col < col_end; col++) {
      ...
      if (letter_px != 0) {
        ...
        if (disp->driver.vdb_wr) {
          disp->driver.vdb_wr(
              (uint8_t *)vdb_p->buf, vdb_width, (col + pos_x) - vdb_p->area.x1,
              (row + pos_y) - vdb_p->area.y1, color, px_opa);
        }
        else {
#if LV_COLOR_SCREEN_TRANSP == 0
          *vdb_buf_tmp = lv_color_mix(color, *vdb_buf_tmp, px_opa);
#else
          *vdb_buf_tmp = color_mix_2_alpha(
              *vdb_buf_tmp, (*vdb_buf_tmp).alpha, color, px_opa);
#endif
        }
      }
 ...
 * @param chroma_keyed true: enable transparency of LV_IMG_LV_COLOR_TRANSP color
 * pixels
 ...
 */
void lv_vmap(
    const lv_area_t *cords_p, const lv_area_t *mask_p, const uint8_t *map_p,
    ...
    lv_opa_t recolor_opa)
{
  ...
  uint8_t px_size_byte =
      alpha_byte ? LV_IMG_PX_SIZE_ALPHA_BYTE : sizeof(lv_color_t);
    ...
    if (disp->driver.vdb_wr) {
      ...
      for (row = masked_a.y1; row <= masked_a.y2; row++) {
        for (col = 0; col < map_useful_w; col++) {
          lv_color_t px_color =
              *((lv_color_t *)&map_p[(uint32_t)col * px_size_byte]);
          disp->driver.vdb_wr(
              (uint8_t *)vdb_p->buf, vdb_width, col + masked_a.x1, row,
              px_color, opa);
        }
    ...
    /*Normal native VDB*/
      ...
      for (row = masked_a.y1; row <= masked_a.y2; row++) {
        ...
        if (lv_disp_is_mem_blend_supported() == false) {
          ...
          lv_disp_mem_blend(
              vdb_buf_tmp, (lv_color_t *)map_p, map_useful_w, opa);
        }
#else
        sw_mem_blend(vdb_buf_tmp, (lv_color_t *)map_p, map_useful_w, opa);
#endif
  ...
  /*In the other cases every pixel need to be checked one-by-one*/
    ...
    for (row = masked_a.y1; row <= masked_a.y2; row++) {
      for (col = 0; col < map_useful_w; col++) {
        ...
        /*Handle chroma key*/
        if (chroma_key && px_color.full == chroma_key_color.full)
          continue;

        /*Re-color the pixel if required*/
        if (recolor_opa != LV_OPA_TRANSP) {
          if (last_img_px.full !=
              px_color.full) { /*Minor acceleration: calculate only for new
                                  colors (save the last)*/
...
#if LV_COLOR_SCREEN_TRANSP == 0
              vdb_buf_tmp[col] =
                  lv_color_mix(px_color, vdb_buf_tmp[col], opa_result);
#else
              vdb_buf_tmp[col] = color_mix_2_alpha(
                  vdb_buf_tmp[col], vdb_buf_tmp[col].alpha, px_color,
                  opa_result);
//                            if(vdb_buf_tmp[col].alpha == LV_OPA_TRANSP) {
//                                /* When it is the first visible pixel on the
//                                transparent screen
//                                 * simlply use this color and set the pixel
//                                 opa as backrounds alpha*/
//                                vdb_buf_tmp[col] = px_color;
//                                vdb_buf_tmp[col].alpha = opa_result;
//                            } else {
//                                /* If already this pixel is already written
...
//
//                                uint16_t opa_tmp = (uint16_t)opa_result +
//                                ((bg_opa * (255 - opa_result)) >> 8);
//                                vdb_buf_tmp[col].alpha = opa_tmp > 0xFF ? 0xFF
//                                : opa_tmp ;
//                            }
#endif
            }
 ...
 * @param mem a memory address. Considered to a rectangular window according to
 * 'mem_area'
    ...
    lv_color_t color, lv_opa_t opa)
{
  ...
  if (disp->driver.vdb_wr) {
    ...
    if (opa == LV_OPA_COVER) {
      ...
      lv_coord_t  copy_size =
          (fill_area->x2 - fill_area->x1 + 1) * sizeof(lv_color_t);
      mem += mem_width;
 ...
 * @return the mixed color. the alpha channel (color.alpha) contains the result
 * alpha
 */
static inline lv_color_t color_mix_2_alpha(
    lv_color_t bg_color, lv_opa_t bg_opa, lv_color_t fg_color, lv_opa_t fg_opa)
