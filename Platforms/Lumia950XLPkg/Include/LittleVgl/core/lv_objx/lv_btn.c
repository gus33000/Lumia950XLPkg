 ...
 **********************/
static bool
                lv_btn_design(lv_obj_t *ddlist, const lv_area_t *mask, lv_design_mode_t mode);
...
lv_obj_t *lv_btn_create(lv_obj_t *par, const lv_obj_t *copy)
{
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_btn);
  if (ancestor_design == NULL)
    ancestor_design = lv_obj_get_design_func(new_btn);
...
void lv_btn_set_ink_in_time(lv_obj_t *btn, uint16_t time)
{
  ...
  (void)btn;  /*Unused*/
  (void)time; /*Unused*/
  LV_LOG_WARN("`lv_btn_set_ink_ink_time` has no effect if LV_BTN_INK_EFEFCT or "
              "USE_LV_ANIMATION is disabled")
#endif
}
...
void lv_btn_set_ink_wait_time(lv_obj_t *btn, uint16_t time)
{
  ...
  LV_LOG_WARN("`lv_btn_set_ink_wait_time` has no effect if LV_BTN_INK_EFEFCT "
              "or USE_LV_ANIMATION is disabled")
#endif
}
...
void lv_btn_set_ink_out_time(lv_obj_t *btn, uint16_t time)
{
  ...
  (void)btn;  /*Unused*/
  (void)time; /*Unused*/
  LV_LOG_WARN("`lv_btn_set_ink_out_time` has no effect if LV_BTN_INK_EFEFCT or "
              "USE_LV_ANIMATION is disabled")
#endif
}
 ...
 */
static bool
lv_btn_design(lv_obj_t *btn, const lv_area_t *mask, lv_design_mode_t mode)
{
  if (mode == LV_DESIGN_COVER_CHK) {
    ...
    if (btn != ink_obj) {
      ...
      if (ink_playback == false) {
        ...
        /*In the first part of the animation increase the size of the circle
         * (ink effect) */
        lv_area_t cir_area;
        ...
        lv_area_intersect(
            &cir_area, &btn->coords,
            &cir_area); /*Limit the area. (It might be too big on the smaller
                           side)*/
        ...
        lv_style_mix(
            ext->styles[ink_bg_state], ext->styles[ink_top_state], &res,
            ink_act_value);
...
static lv_res_t lv_btn_signal(lv_obj_t *btn, lv_signal_t sign, void *param)
{
  ...
  if (sign == LV_SIGNAL_PRESSED) {
    ...
    /*If not a toggle button and the "IN" inking is ready then start an "OUT"
     * inking*/
    else if (ink_ready && ext->ink_out_time > 0) {
      ink_obj = btn;
      ink_playback =
          true; /*It is the playback. If not set `lv_btn_ink_effect_anim_ready`
    ...
    if (c == LV_GROUP_KEY_RIGHT || c == LV_GROUP_KEY_UP) {
      ...
      if (ext->actions[LV_BTN_ACTION_CLICK] &&
          lv_btn_get_state(btn) != LV_BTN_STATE_INA) {
        res = ext->actions[LV_BTN_ACTION_CLICK](btn);
      }
    }
