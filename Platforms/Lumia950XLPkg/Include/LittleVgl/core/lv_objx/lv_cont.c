...
lv_obj_t *lv_cont_create(lv_obj_t *par, const lv_obj_t *copy)
{
  ...
  if (ancestor_signal == NULL)
    ancestor_signal = lv_obj_get_signal_func(new_cont);
 ...
 * The container size will be set to involve the children horizontally or
 * vertically.
 * @param cont pointer to a container object
...
void lv_cont_set_fit(lv_obj_t *cont, bool hor_en, bool ver_en)
{
  ...
  lv_cont_ext_t *ext = lv_obj_get_ext_attr(cont);
  if (ext->hor_fit == hor_en && ext->ver_fit == ver_en)
    return;
...
static lv_res_t lv_cont_signal(lv_obj_t *cont, lv_signal_t sign, void *param)
{
  ...
  if (sign ==
      LV_SIGNAL_STYLE_CHG) { /*Recalculate the padding if the style changed*/
...
static void lv_cont_refr_layout(lv_obj_t *cont)
{
  ...
  if (type == LV_LAYOUT_CENTER) {
    lv_cont_layout_center(cont);
      ...
      type == LV_LAYOUT_COL_L || type == LV_LAYOUT_COL_M ||
      type == LV_LAYOUT_COL_R) {
    lv_cont_layout_col(cont);
  }
  else if (
      type == LV_LAYOUT_ROW_T || type == LV_LAYOUT_ROW_M ||
      type == LV_LAYOUT_ROW_B) {
...
static void lv_cont_layout_col(lv_obj_t *cont)
{
  ...
  LL_READ_BACK(cont->child_ll, child)
  {
    if (lv_obj_get_hidden(child) != false ||
        lv_obj_is_protected(child, LV_PROTECT_POS) != false)
      continue;
...
static void lv_cont_layout_center(lv_obj_t *cont)
{
  ...
  LL_READ_BACK(cont->child_ll, child)
  {
    ...
    lv_obj_align(
        child, cont, LV_ALIGN_CENTER, 0,
        last_cord + lv_obj_get_height(child) / 2);
    last_cord += lv_obj_get_height(child) + style->body.padding.inner;
  }
...
static void lv_cont_layout_pretty(lv_obj_t *cont)
{
  ...
  while (child_rs != NULL) {
    lv_coord_t h_row = 0;
    lv_coord_t w_row = style->body.padding.hor *
                       2; /*The width is at least the left+right hpad*/
    uint32_t obj_num = 0;
    ...
    do {
      if (lv_obj_get_hidden(child_rc) == false &&
        ...
        if (w_row + lv_obj_get_width(child_rc) > w_obj) {
          /*Step back one child because the last already not fit, so the
           * previous is the closer*/
        ...
        h_row = LV_MATH_MAX(
            h_row, lv_obj_get_height(child_rc)); /*Search the highest object*/
        obj_num++;
        if (lv_obj_is_protected(child_rc, LV_PROTECT_FOLLOW))
          break; /*If can not be followed by an other object then break here*/
      }
      child_rc =
          lv_ll_get_prev(&cont->child_ll, child_rc); /*Load the next object*/
      if (obj_num == 0)
        child_rs = child_rc; /*If the first object was hidden (or too long) then
                                set the next as first */
    } while (child_rc != NULL);
    ...
    if (obj_num == 0) {
      if (child_rc != NULL) {
        ...
        h_row = lv_obj_get_height(
            child_rc); /*Not set previously because of the early break*/
      }
    }
    ...
    /*If there are two object in the row then align them proportionally*/
      ...
      lv_obj_align(
          obj1, cont, LV_ALIGN_IN_TOP_LEFT, pad,
          act_y + (h_row - lv_obj_get_height(obj1)) / 2);
      lv_obj_align(
          obj2, cont, LV_ALIGN_IN_TOP_RIGHT, -pad,
    ...
    /* Align the children (from child_rs to child_rc)*/
      ...
      while (child_tmp != NULL) {
        if (lv_obj_get_hidden(child_tmp) == false &&
          ...
          lv_obj_align(
              child_tmp, cont, LV_ALIGN_IN_TOP_LEFT, act_x,
    ...
    child_rs =
        lv_ll_get_prev(&cont->child_ll, child_rc); /*Go to the next object*/
    child_rc = child_rs;
  }
  lv_obj_clear_protect(cont, LV_PROTECT_CHILD_CHG);
}
...
static void lv_cont_layout_grid(lv_obj_t *cont)
{
  ...
  uint16_t    obj_row = (w_tot - (2 * style->body.padding.hor)) /
                     (w_obj + style->body.padding.inner); /*Obj. num. in a row*/
  lv_coord_t x_ofs;
  if (obj_row > 1) {
    x_ofs =
        w_obj + (w_tot - (2 * style->body.padding.hor) - (obj_row * w_obj)) /
                    (obj_row - 1);
  }
  else {
...
static void lv_cont_refr_autofit(lv_obj_t *cont)
{
  ...
  if (ext->hor_fit == 0 && ext->ver_fit == 0) {
    return;
  }
  ...
  if (new_cords.x1 != LV_COORD_MAX) {
    ...
    if (cont->coords.x1 != new_cords.x1 || cont->coords.y1 != new_cords.y1 ||
        cont->coords.x2 != new_cords.x2 || cont->coords.y2 != new_cords.y2) {
