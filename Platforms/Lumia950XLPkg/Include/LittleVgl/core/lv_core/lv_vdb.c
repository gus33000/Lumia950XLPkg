...
/*Double VDB*/
static volatile lv_vdb_state_t vdb_state[2] = {LV_VDB_STATE_FREE,
                                               LV_VDB_STATE_FREE};
#if LV_VDB_ADR == 0
/*If the buffer address is not specified  simply allocate it*/
static uint8_t  vdb_buf1[LV_VDB_SIZE_IN_BYTES];
static uint8_t  vdb_buf2[LV_VDB_SIZE_IN_BYTES];
static lv_vdb_t vdb[2] = {{.buf = (lv_color_t *)vdb_buf1},
                          {.buf = (lv_color_t *)vdb_buf2}};
#else /*LV_VDB_ADR != 0*/
/*If the buffer address is specified use that address*/
static lv_vdb_t vdb[2] = {{.buf = (lv_color_t *)LV_VDB_ADR},
                          {.buf = (lv_color_t *)LV_VDB2_ADR}};
#endif
#endif
...
lv_vdb_t *lv_vdb_get(void)
{
  ...
  if (vdb.buf == (void *)LV_VDB_ADR_INV) {
    LV_LOG_ERROR("VDB address is invalid. Use `lv_vdb_set_adr` to set a valid "
                 "address or use LV_VDB_ADR = 0 in lv_conf.h");
    return NULL;
  }
  return &vdb;
#else
  /*If already there is an active do nothing*/
  if (vdb_state[0] == LV_VDB_STATE_ACTIVE)
    return &vdb[0];
  if (vdb_state[1] == LV_VDB_STATE_ACTIVE)
    return &vdb[1];
...
void lv_vdb_flush(void)
{
...
#if LV_VDB_DOUBLE == 0
  vdb_state = LV_VDB_STATE_FLUSH; /*User call to 'lv_flush_ready()' will set to
                                     ACTIVE 'disp_flush'*/
#else
  /* Wait the pending flush before starting this one
   * (Don't forget: 'lv_flush_ready()' has to be called when flushing is
   * ready)*/
  while (vdb_state[0] == LV_VDB_STATE_FLUSH ||
         vdb_state[1] == LV_VDB_STATE_FLUSH)
    ;

  /*Turn the active VDB to flushing*/
  if (vdb_state[0] == LV_VDB_STATE_ACTIVE)
    vdb_state[0] = LV_VDB_STATE_FLUSH;
  if (vdb_state[1] == LV_VDB_STATE_ACTIVE)
    vdb_state[1] = LV_VDB_STATE_FLUSH;
#endif
  ...
  lv_disp_flush(
      vdb_act->area.x1, vdb_act->area.y1, vdb_act->area.x2, vdb_act->area.y2,
      vdb_act->buf);
}

/**
 * Set the address of VDB buffer(s) manually. To use this set `LV_VDB_ADR` (and
 * `LV_VDB2_ADR`) to `LV_VDB_ADR_INV` in `lv_conf.h`. It should be called before
 * `lv_init()`. The size of the buffer should be: `LV_VDB_SIZE_IN_BYTES`
...
/**
 * Call in the display driver's  'disp_flush' function when the flushing is
 * finished
 */
 ...
 */
void lv_flush_ready(void) { /*Do nothing. It is used only for VDB*/ }
#endif
