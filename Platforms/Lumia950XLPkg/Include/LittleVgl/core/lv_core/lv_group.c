...
void lv_group_del(lv_group_t *group)
{
  ...
  if (group->obj_focus != NULL) {
    (*group->obj_focus)
        ->signal_func(*group->obj_focus, LV_SIGNAL_DEFOCUS, NULL);
    lv_obj_invalidate(*group->obj_focus);
  }

  /*Remove the objects from the group*/
  lv_obj_t **obj;
  LL_READ(group->obj_ll, obj) { (*obj)->group_p = NULL; }
...
void lv_group_remove_obj(lv_obj_t *obj)
{
  ...
  if (g == NULL)
    return;
  if (g->obj_focus == NULL)
    return; /*Just to be sure (Not possible if there is at least one object in
               the group)*/
   ...
   * group but it will be deleted. Set the `obj_focus` to NULL to get back to
   * the initial state of the group with zero objects*/
  if (*g->obj_focus == obj) {
    g->obj_focus = NULL;
  }
...
void lv_group_focus_obj(lv_obj_t *obj)
{
  ...
  LL_READ(g->obj_ll, i)
  {
    if (*i == obj) {
      if (g->obj_focus == i)
        return; /*Don't focus the already focused object again*/
      if (g->obj_focus != NULL) {
...
void lv_group_focus_next(lv_group_t *group)
{
  ...
  lv_obj_t **obj_next;
  if (group->obj_focus == NULL)
    obj_next = lv_ll_get_head(&group->obj_ll);
  else
    obj_next = lv_ll_get_next(&group->obj_ll, group->obj_focus);
...
void lv_group_focus_prev(lv_group_t *group)
{
  ...
  lv_obj_t **obj_next;
  if (group->obj_focus == NULL)
    obj_next = lv_ll_get_tail(&group->obj_ll);
  else
    obj_next = lv_ll_get_prev(&group->obj_ll, group->obj_focus);
 ...
 */
void lv_group_set_style_mod_cb(
    lv_group_t *group, lv_group_style_mod_func_t style_mod_func)
{
 ...
 * @param style_mod_func the style modifier function pointer
 */
void lv_group_set_style_mod_edit_cb(
    lv_group_t *group, lv_group_style_mod_func_t style_mod_func)
{
...
void lv_group_set_editing(lv_group_t *group, bool edit)
{
  ...
  if (focused)
    focused->signal_func(
        focused, LV_SIGNAL_FOCUS,
        NULL); /*Focus again to properly leave edit mode*/
...
lv_style_t *lv_group_mod_style(lv_group_t *group, const lv_style_t *style)
{
  ...
  if (group->editing) {
    if (group->style_mod_edit != NULL)
      group->style_mod_edit(&group->style_tmp);
    else
      style_mod_edit_def(&group->style_tmp);
  }
  else {
    if (group->style_mod != NULL)
      group->style_mod(&group->style_tmp);
    else
      style_mod_def(&group->style_tmp);
  }
...
static void style_mod_def(lv_style_t *style)
{
  ...
  style->body.main_color =
      lv_color_mix(style->body.main_color, LV_COLOR_ORANGE, LV_OPA_70);
  style->body.grad_color =
      lv_color_mix(style->body.grad_color, LV_COLOR_ORANGE, LV_OPA_70);
  style->body.shadow.color =
      lv_color_mix(style->body.shadow.color, LV_COLOR_ORANGE, LV_OPA_60);

  style->text.color =
      lv_color_mix(style->text.color, LV_COLOR_ORANGE, LV_OPA_70);
#else
  style->body.border.opa   = LV_OPA_COVER;
...
static void style_mod_edit_def(lv_style_t *style)
{
  ...
  style->body.main_color =
      lv_color_mix(style->body.main_color, LV_COLOR_GREEN, LV_OPA_70);
  style->body.grad_color =
      lv_color_mix(style->body.grad_color, LV_COLOR_GREEN, LV_OPA_70);
  style->body.shadow.color =
      lv_color_mix(style->body.shadow.color, LV_COLOR_GREEN, LV_OPA_60);

  style->text.color =
      lv_color_mix(style->text.color, LV_COLOR_GREEN, LV_OPA_70);
#else
  style->body.border.opa   = LV_OPA_COVER;
  style->body.border.color = LV_COLOR_BLACK;
