...
#define LV_RADIUS_CIRCLE                                                       \
  (LV_COORD_MAX) /*A very big radius to always draw as circle*/
 ...
 */
void lv_style_mix(
    const lv_style_t *start, const lv_style_t *end, lv_style_t *res,
    uint16_t ratio);
 ...
 * @param anim pointer to a pre-configured 'lv_style_anim_t' variable (will be
 * copied)
 * @return pointer to a descriptor. Really this variable will be animated. (Can
 * be used in `lv_anim_del(dsc, NULL)`)
 */
