 ...
 **********************/
static void
            refresh_childen_position(lv_obj_t *obj, lv_coord_t x_diff, lv_coord_t y_diff);
static void report_style_mod_core(void *style_p, lv_obj_t *obj);
static void refresh_childen_style(lv_obj_t *obj);
static void delete_children(lv_obj_t *obj);
static bool
                lv_obj_design(lv_obj_t *obj, const lv_area_t *mask_p, lv_design_mode_t mode);
...
void lv_obj_invalidate(const lv_obj_t *obj)
{
  ...
  if (obj_scr == lv_scr_act() || obj_scr == lv_layer_top() ||
    ...
    while (par != NULL) {
      ...
      if (union_ok == false)
        break; /*If no common parts with parent break;*/
      if (lv_obj_get_hidden(par))
        return; /*If the parent is hidden then the child is hidden and won't be
                   drawn*/
 ...
 */
void lv_obj_align(
    lv_obj_t *obj, const lv_obj_t *base, lv_align_t align, lv_coord_t x_mod,
    lv_coord_t y_mod)
{
...
void lv_obj_set_hidden(lv_obj_t *obj, bool en)
{
  if (!obj->hidden)
    lv_obj_invalidate(
        obj); /*Invalidate when not hidden (hidden objects are ignored) */
 ...
 */
void lv_obj_set_top(lv_obj_t *obj, bool en) { obj->top = (en == true ? 1 : 0); }
 ...
 */
void lv_obj_set_protect(lv_obj_t *obj, uint8_t prot) { obj->protect |= prot; }
 ...
 */
void lv_obj_animate(
    lv_obj_t *obj, lv_anim_builtin_t type, uint16_t time, uint16_t delay,
    void (*cb)(lv_obj_t *))
{
  lv_obj_t *par = lv_obj_get_parent(obj);
 ...
 */
lv_obj_t *lv_scr_act(void) { return act_scr; }
 ...
 */
lv_obj_t *lv_layer_top(void) { return top_layer; }
 ...
 */
lv_obj_t *lv_layer_sys(void) { return sys_layer; }
 ...
 */
lv_obj_t *lv_obj_get_parent(const lv_obj_t *obj) { return obj->par; }
 ...
 */
lv_coord_t lv_obj_get_ext_size(const lv_obj_t *obj) { return obj->ext_size; }
...
lv_opa_t lv_obj_get_opa_scale(const lv_obj_t *obj)
{
  ...
  while (parent) {
    if (parent->opa_scale_en)
      return parent->opa_scale;
 ...
 */
uint8_t lv_obj_get_protect(const lv_obj_t *obj) { return obj->protect; }
 ...
 */
void *lv_obj_get_ext_attr(const lv_obj_t *obj) { return obj->ext_attr; }
 ...
 */
void *lv_obj_get_free_ptr(const lv_obj_t *obj) { return obj->free_ptr; }
#endif
 ...
 */
void *lv_obj_get_group(const lv_obj_t *obj) { return obj->group_p; }
...
lv_obj_design(lv_obj_t *obj, const lv_area_t *mask_p, lv_design_mode_t mode)
{
  if (mode == LV_DESIGN_COVER_CHK) {
    ...
    area_tmp.x1 += r;
    area_tmp.x2 -= r;
    if (lv_area_is_in(mask_p, &area_tmp) == false)
      return false;
...
static lv_res_t lv_obj_signal(lv_obj_t *obj, lv_signal_t sign, void *param)
{
  ...
  if (sign == LV_SIGNAL_CHILD_CHG) {
    ...
    if (style->body.shadow.width > obj->ext_size)
      obj->ext_size = style->body.shadow.width;
  }
