 ...
 * called periodically. A priority (5 levels + disable) can be assigned to
 * lv_tasks.
 */
...
LV_ATTRIBUTE_TASK_HANDLER void lv_task_handler(void)
{
   ...
   * priority but on the priority of executed tasks don't run tasks before the
   * executed*/
  ...
  do {
    ...
    while (task_act) {
      ...
      if (task_act == task_interrupter) {
        task_interrupter =
            NULL; /*From this point only task after the interrupter comes, so
      ...
      /*Tasks with higher priority then the interrupted shall be run in every
       * case*/
      else if (task_interrupter) {
        if (task_act->prio > task_interrupter->prio) {
          if (lv_task_exec(task_act)) {
            task_interrupter =
                task_act; /*Check all tasks again from the highest priority */
            end_flag = false;
            break;
          }
        }
      ...
      if (task_deleted)
        break; /*If a task was deleted then this or the next item might be
                  corrupted*/
      if (task_created)
        break; /*If a task was deleted then this or the next item might be
                  corrupted*/
 ...
 */
lv_task_t *lv_task_create(
    void (*task)(void *), uint32_t period, lv_task_prio_t prio, void *param)
{
...
void lv_task_set_prio(lv_task_t *lv_task_p, lv_task_prio_t prio)
{
  ...
  LL_READ(lv_task_ll, i)
  {
    if (i->prio <= prio) {
      if (i != lv_task_p)
        lv_ll_move_before(&lv_task_ll, lv_task_p, i);
      break;
    }
  }
 ...
 * @param lv_task_p pointer to a lv_task.
 */
void lv_task_once(lv_task_t *lv_task_p) { lv_task_p->once = 1; }
 ...
 * suspended
 */
void lv_task_enable(bool en) { lv_task_run = en; }

/**
 * Get idle percentage
 * @return the lv_task idle in percentage
 */
uint8_t lv_task_get_idle(void) { return idle_last; }
