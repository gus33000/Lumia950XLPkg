...
void lv_txt_get_size(
    lv_point_t *size_res, const char *text, const lv_font_t *font,
 ...
 */
uint16_t lv_txt_get_next_line(
    const char *txt, const lv_font_t *font_p, lv_coord_t letter_space,
    lv_coord_t max_l, lv_txt_flag_t flag);

/**
 * Give the length of a text with a given font
 * @param txt a '\0' terminate string
 * @param length length of 'txt' in character count and not bytes(UTF-8 can be
 ...
 */
lv_coord_t lv_txt_get_width(
    const char *txt, uint16_t length, const lv_font_t *font,
