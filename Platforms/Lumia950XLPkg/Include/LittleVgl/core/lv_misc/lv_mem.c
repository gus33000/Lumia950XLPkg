...
#define LV_MEM_ADD_JUNK                                                        \
  0 /*Add memory junk on alloc (0xaa) and free(0xbb) (just for testing         \
       purposes)*/
...
typedef struct {
  ...
  uint8_t first_data; /*First data byte in the allocated data (Just for easily
                         create a pointer)*/
} lv_mem_ent_t;
 ...
 **********************/
#if LV_MEM_CUSTOM == 0
static LV_MEM_ATTR MEM_UNIT
                   work_mem_int[LV_MEM_SIZE / sizeof(MEM_UNIT)]; /*Work memory for
                                                                    allocations*/
...
void *lv_mem_realloc(void *data_p, uint32_t new_size)
{
  ...
  if (data_p != NULL) {
    lv_mem_ent_t *e =
        (lv_mem_ent_t *)((uint8_t *)data_p - sizeof(lv_mem_header_t));
    if (e->header.used == 0) {
      data_p = NULL;
    }
  }
   ...
   * If the 'old_size' was extended by a header size in 'ent_trunc' it avoids
   * reallocating this same memory */
...
void lv_mem_monitor(lv_mem_monitor_t *mon_p)
{
  ...
  mon_p->frag_pct =
      (uint32_t)mon_p->free_biggest_size * 100U / mon_p->free_size;
...
static void ent_trunc(lv_mem_ent_t *e, uint32_t size)
{
  ...
  if (e->header.d_size != size) {
    ...
    after_new_e->header.d_size =
        e->header.d_size - size - sizeof(lv_mem_header_t);
  }
