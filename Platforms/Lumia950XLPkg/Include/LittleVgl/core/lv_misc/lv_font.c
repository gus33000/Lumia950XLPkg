 ...
 */
void lv_font_init(void) { lv_font_builtin_init(); }

/**
 * Create a pair from font name and font dsc. get function. After it 'font_get'
 * can be used for this font
...
void lv_font_add(lv_font_t *child, lv_font_t *parent)
{
  ...
  while (parent->next_page != NULL) {
    parent =
        parent->next_page; /*Got to the last page and add the new font there*/
  }
...
/**
 * Get the width of a letter in a font. If `monospace` is set then return with
 * it.
 * @param font_p pointer to a font
 * @param letter an UNICODE character code
...
/**
 * Get the width of the letter without overwriting it with the `monospace`
 * attribute
 * @param font_p pointer to a font
 * @param letter an UNICODE character code
 * @return the width of a letter
...
/**
 * Generic bitmap get function used in 'font->get_bitmap' when the font contains
 ...
 */
const uint8_t *
lv_font_get_bitmap_continuous(const lv_font_t *font, uint32_t unicode_letter)
{
  /*Check the range*/
  if (unicode_letter < font->unicode_first ||
      unicode_letter > font->unicode_last)
    return NULL;
...
/**
 * Generic bitmap get function used in 'font->get_bitmap' when the font NOT
 * contains all characters in the range (sparse)
 * @param font pointer to font
 * @param unicode_letter an unicode letter which bitmap should be get
 * @return pointer to the bitmap or NULL if not found
 */
const uint8_t *
lv_font_get_bitmap_sparse(const lv_font_t *font, uint32_t unicode_letter)
{
  /*Check the range*/
  if (unicode_letter < font->unicode_first ||
      unicode_letter > font->unicode_last)
...
/**
 * Generic glyph width get function used in 'font->get_width' when the font
 * contains all characters in the range
 * @param font pointer to font
 * @param unicode_letter an unicode letter which width should be get
 * @return width of the gylph or -1 if not found
 */
int16_t
lv_font_get_width_continuous(const lv_font_t *font, uint32_t unicode_letter)
{
  /*Check the range*/
  if (unicode_letter < font->unicode_first ||
      unicode_letter > font->unicode_last) {
...
/**
 * Generic glyph width get function used in 'font->get_bitmap' when the font NOT
 * contains all characters in the range (sparse)
 * @param font pointer to font
 * @param unicode_letter an unicode letter which width should be get
