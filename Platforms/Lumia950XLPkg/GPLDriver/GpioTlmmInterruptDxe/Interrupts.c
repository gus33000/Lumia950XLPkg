...
{
  return PcdGet64(PcdGpioTlmmIntrCfgOffset) +
         PcdGet64(PcdGpioTlmmIntrCfgElementSize) * id;
}

STATIC inline UINTN gpio_tlmm_platform_intr_status_reg(UINTN id)
{
  return PcdGet64(PcdGpioTlmmIntrStatusOffset) +
         PcdGet64(PcdGpioTlmmIntrStatusElementSize) * id;
}

STATIC inline UINTN gpio_tlmm_platform_intr_target_reg(UINTN id)
{
  return PcdGet64(PcdGpioTlmmIntrTargetOffset) +
         PcdGet64(PcdGpioTlmmIntrTargetElementSize) * id;
}

STATIC inline UINTN gpio_tlmm_platform_io_reg(UINTN id)
{
  return PcdGet64(PcdGpioTlmmIoOffset) +
         PcdGet64(PcdGpioTlmmIoElementSize) * id;
}
...
STATIC VOID msm_gpio_update_dual_edge_pos(UINTN id)
{
  ...
  do {
    val =
        readl(
            PcdGet64(PcdGpioTlmmBaseAddress) + gpio_tlmm_platform_io_reg(id)) &
        BIT(PcdGet64(PcdGpioTlmmInBit));

    pol = readl(
        PcdGet64(PcdGpioTlmmBaseAddress) + gpio_tlmm_platform_intr_cfg_reg(id));
    ...
    val2 =
        ...
        BIT(PcdGet64(PcdGpioTlmmInBit));
    intstat = readl(
        PcdGet64(PcdGpioTlmmBaseAddress) +
        gpio_tlmm_platform_intr_status_reg(id));
    if (intstat || (val == val2))
      return;
  } while (loop_limit-- > 0);

  DEBUG(
      (EFI_D_ERROR, "dual-edge irq failed to stabilize, %08x != %08x\n", val,
       val2));
}
...
VOID msm_gpio_irq_ack(UINTN id)
{
  ...
  if (PcdGetBool(PcdGpioTlmmIntrAckHigh))
    ...
    val &= ~BIT(PcdGet64(PcdGpioTlmmIntrStatusBit));
  writel(
      val, PcdGet64(PcdGpioTlmmBaseAddress) +
               gpio_tlmm_platform_intr_status_reg(id));
...
EFI_STATUS msm_gpio_irq_set_type(UINTN id, UINTN type)
{
  ...
  //
  if (PcdGet64(PcdGpioTlmmIntrDetectionWidth) == 1 &&
      type == TLMM_IRQ_TYPE_EDGE_BOTH)
  ...
  val = readl(
      PcdGet64(PcdGpioTlmmBaseAddress) +
      gpio_tlmm_platform_intr_target_reg(id));
  val &= ~(7 << PcdGet64(PcdGpioTlmmIntrTargetBit));
  val |= PcdGet64(PcdGpioTlmmIntrTargetKpssValue)
         << PcdGet64(PcdGpioTlmmIntrTargetBit);
  writel(
      val, PcdGet64(PcdGpioTlmmBaseAddress) +
               gpio_tlmm_platform_intr_target_reg(id));
...
STATIC enum handler_return msm_gpio_irq_handler(VOID *arg)
{
  ...
  for (i = 0; i < MAX_NR_GPIO; i++) {
    ...
    val = readl(
        PcdGet64(PcdGpioTlmmBaseAddress) +
        gpio_tlmm_platform_intr_status_reg(i));
...
VOID msm_gpio_irq_init(VOID)
{
  ...
  register_int_handler(
      PcdGet64(PcdGpioTlmmSummaryIrq), msm_gpio_irq_handler, 0);
